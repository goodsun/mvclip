...
export async function transcribeVideo(
  ...
  try {
    ...
    if (timeRange) {
      ...
      console.log(
        `⏱️ 解析範囲: ${startSeconds}秒 〜 ${endSeconds}秒 (${duration}秒間)`
      );
    }
    ...
    console.log(
      `🔧 使用中の圧縮設定: ${getCompressionSettings(compressionLevel).name}`
    );
    ...
    await retryWithBackoff(
      async () => {
        try {
          ...
          console.log(
            `✅ 音声抽出完了: ${(audioStats.size / (1024 * 1024)).toFixed(
              2
            )} MB`
          );
        } catch (error) {
    ...
    const compressionRatio = (
      ((originalSizeMB - audioSizeMB) / originalSizeMB) *
      100
    ).toFixed(1);
    ...
    console.log(
      `  総時間: ${
        transcription.duration ? transcription.duration.toFixed(3) : "N/A"
      }秒`
    );
    ...
    console.log(`  音声ファイル実際の長さ: ${actualAudioDuration.toFixed(3)}秒`);
    
    if (transcription.duration) {
      const diff = actualAudioDuration - transcription.duration;
      console.log(`  ⚠️ 時間差: ${diff.toFixed(3)}秒 (${(diff / actualAudioDuration * 100).toFixed(2)}%)`);
    }
    ...
    if (transcription.segments && transcription.segments.length > 0) {
      ...
      transcription.segments.slice(0, 3).forEach((seg, i) => {
        console.log(
          `    [${i + 1}] 生の値: start=${seg.start}, end=${seg.end}`
        );
        console.log(
          `        フォーマット: ${seg.start.toFixed(3)}s - ${seg.end.toFixed(
            3
          )}s: "${seg.text}"`
        );
      });
      ...
      transcription.segments.slice(-3).forEach((seg, i) => {
        ...
        console.log(
          `    [${index}] 生の値: start=${seg.start}, end=${seg.end}`
        );
        console.log(
          `        フォーマット: ${seg.start.toFixed(3)}s - ${seg.end.toFixed(
            3
    ...
    if (transcription.words && transcription.words.length > 0) {
      ...
      transcription.words.slice(0, 3).forEach((word, i) => {
        console.log(
          `    [${i + 1}] ${word.start.toFixed(3)}s - ${word.end.toFixed(
            3
          )}s: "${word.word}"`
        );
      });
...
async function transcribeLargeAudio(audioPath) {
  try {
    ...
    console.log(`📏 音声ファイルの総時間: ${totalDuration.toFixed(3)}秒`);

    // 音声ファイルを分割
    const segmentDir = path.dirname(audioPath) + "/segments";
    fs.mkdirSync(segmentDir, { recursive: true });

    // 6分ごとに分割（キーフレームで分割して正確な時間を保持）
    await execAsync(
      `ffmpeg -i "${audioPath}" -f segment -segment_time 360 -c copy -reset_timestamps 0 "${segmentDir}/segment_%03d.mp3"`
    );
    ...
    for (let i = 0; i < segments.length; i++) {
      const segmentPath = path.join(segmentDir, segments[i]);
      
      // セグメントの開始時刻を計算（6分 × セグメント番号）
      const expectedStartTime = i * 360;
      segmentOffsets.push(expectedStartTime);
      
      console.log(`セグメント処理中: ${segments[i]} (開始時刻: ${expectedStartTime}秒)`);
      ...
      // タイムスタンプを調整（セグメントの開始時刻を加算）
      if (transcription.segments) {
        transcription.segments.forEach((seg) => {
          seg.start += expectedStartTime;
          seg.end += expectedStartTime;
    ...
    if (allSegments.length > 0) {
      ...
      console.log(`🔍 最後のセグメント: ${lastSegment.end.toFixed(3)}秒 / 総時間: ${totalDuration.toFixed(3)}秒`);
      
      // タイムスタンプのドリフトを修正
      const drift = totalDuration / lastSegment.end;
      if (Math.abs(drift - 1) > 0.01) {
        console.log(`⚠️ タイムドリフト検出: ${((drift - 1) * 100).toFixed(2)}%`);
        console.log(`🔧 タイムスタンプを補正中...`);
...
function fillSilentGaps(transcription, videoDuration) {
  ...
  if (lastSegment.end < videoDuration - 0.01) {
    // より精密な閾値
    filledSegments.push({
  ...
  console.log(
    `📝 無音部分補完: ${segments.length}個 → ${filledSegments.length}個のセグメント`
  );
  console.log(
    `📹 動画長: ${videoDuration.toFixed(
      3
    )}秒, 最後のセグメント: ${filledSegments[
      filledSegments.length - 1
    ].end.toFixed(3)}秒`
  );
  ...
  filledSegments.slice(0, 3).forEach((seg, i) => {
    ...
    console.log(
      `  セグメント${i + 1}: ${seg.start.toFixed(3)}s - ${seg.end.toFixed(
        3
      )}s: ${text}`
    );
  });
